...
function DashboardScreen() {
  ...
  const addChatComponent = () => {
    setChatCount((prev) => {
      ...
      toast({ title: "Uh oh!", description: "You can only have 6 chat components", variant: "destructive" });
      return prev;
    });
  };
  ...
  const removeChatComponent = () => {
    setChatCount((prev) => {
      ...
      toast({ title: "Uh oh!", description: "You need at least 1 chat component", variant: "destructive" });
      return prev;
    });
  };


  const handleSyncMessageChange = (event: ChangeEvent<HTMLInputElement>) => setSyncMessage(event.target.value);
  ...
  const getGridClasses = (count: number) => {
    ...
    return `${baseClasses} ${
      count === 1 ? "grid-cols-1 sm:grid-cols-1" :
      ...
      <div className="bg-gray-200 w-full h-screen p-2 pb-6">
        ...
        <div className="flex items-center m-2 justify-end">
          <div className="flex items-center">
            <Button onClick={addChatComponent} className="hover:bg-gray-300 bg-transparent text-gray-800 hidden md:block">
              <IconTablePlus />
            </Button>
            <Button onClick={removeChatComponent} className="bg-transparent hover:bg-gray-300 text-gray-800 hidden md:block">
              <IconTableMinus />
            </Button>
            <p className="mx-2 hidden md:block text-gray-400">Sync Chat</p>
            <CustomSwitch isSyncChat={isSyncChat} onToggle={handleSyncChatToggle} />
          </div>
        </div>
        <div className="w-full">
          <div className={`grid gap-x-4 justify-start items-start w-full p-4 h-[80vh] overflow-y-scroll bg-gray-200 ${getGridClasses(chatCount)}`}>
          ...
          {isSyncChat && (
            <div className="mt-4 justify-center items-center mx-8 hidden md:flex">
              ...
              />
              <Button onClick={handleSendButtonClick} className="bg-gray-600 text-white mx-2">
                <IconSend />
              </Button>
            </div>
          )}
...
const CustomSwitch = ({ isSyncChat, onToggle }: { isSyncChat: boolean, onToggle: () => void }) => (
